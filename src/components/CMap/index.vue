...
<script>
import { home as mapBus } from "@/map/cesium";
import { constant } from "@/map";
import { getProviderViewModels } from "@/map/cesium/threeD/provider";


const { DEFAULT_VIEW_INFO } = constant;
  ...
  methods: {
    init() {
      ...
      Cesium.Ion.defaultAccessToken =
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiJmYjU0MGUyNS03YmUzLTQ4MjQtODI1MC1iNzcxMGUyMjY5MzUiLCJpZCI6MjEzMzU0LCJpYXQiOjE3MTUwNDU1OTF9.OWQEv7Vz218FVCUQqr3VxHNBxYKqx3W_vY9WZSUJfus";
    ...
    async getData() {
      ...
      const f = await fetch(
        "http://10.253.102.69/gw/TILE_3D_MODEL/sz/dapeng/tileset.json",
        {
          method: "GET",
          headers: headers2,
          mode: "cors",
    ...
    viewDam2() {
      ...
      viewer.scene.camera.flyTo({
        //将经度、纬度、高度的坐标转换为笛卡尔坐标
        destination: Cesium.Cartesian3.fromDegrees(114.402396, 25.8297292, 100),
        orientation: {
          heading: 5.263221487362526,
          pitch: -0.48865517523326973,
          roll: 6.283148434762025,
        },
      });
    },
  },
  ...
  async mounted1() {
    ...
    try {
      ...
      var promise = scene.open(
        "http://113.108.111.178:8081/iserver/services/3D-scene/rest/realspace"
      );
