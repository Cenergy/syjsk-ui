<template>
  ...
  >
    ...
    <el-form label-width="80px" label-position="right" size="mini">
      <el-form-item label="选中水位" style="color: #fff !important">
      ...
      <div style="width: 80%; margin: 30px 45px">
        <MySlider
...
export default {
  ...
  data() {
    return {
      ...
      waterLevelColorSegments: [
        { start: 183, end: 198.4, color: "#3c4ae6ff" }, // 安全水位 - 绿色
      ...
      // 参数范围定义
      rangeParams: {
        waterLevel: [183, 202.5], // 显示范围：150-300，使用 WaterLayer.convertHeightToWaterLevel 转换为 waterLevel 参数
        waveHeight: [0.0, 1.0],
        waveSpeed: [0.001, 0.1],
  ...
  methods: {
    ...
    async loadEffectLayer(waterLevel) {
      if (![199.1, 199.6, 200.1, 200.6].includes(Number(waterLevel))) {
