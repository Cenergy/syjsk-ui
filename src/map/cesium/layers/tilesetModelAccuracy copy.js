...
class tilesetModelAccuracy extends BaseLayer {
  ...
  async show() {
    ...
    function tileSet(tileset, center, height = 10) {
      tileset.readyPromise.then(() => {
        const position = Cesium.Cartesian3.fromDegrees(
          item.center[0],
          item.center[1],
          item.center[2] || height
        );
        var mat=Cesium.Transforms.eastNorthUpToFixedFrame(position);
        var scale = Cesium.Matrix4.fromUniformScale(paramas.scale);
        Cesium.Matrix4.multiply(mat, scale, mat);
        var mx=Cesium.Matrix3.fromRotationX(Cesium.Math.toRadians(paramas.rx));
        Cesium.Matrix4.multiply(mat, mx, mat);
        mx=Cesium.Matrix4.fromRotationY(paramas.ry);
        Cesium.Matrix4.multiply(mat, mx, mat);
        mx=Cesium.Matrix4.fromRotationZ(paramas.rz);
        Cesium.Matrix4.multiply(mat, mx, mat);

        // tileSet(tileset, center, height);
      });
    }
    ...
    for (let i = 0; i < showList.length; i++) {
      ...
      console.log(
        `🚀 ~ tilesetModelAccuracy ~ show ~ ${item.name} resource:`,
        resource
      );
