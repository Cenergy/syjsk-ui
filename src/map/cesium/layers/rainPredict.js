...
class rainPredict extends BaseLayer {
  constructor(options) {
    super(options);
    this.rainPredictEntity = null;
    this.intervalId = null;
    this.currentIndex = 0;
    this.rainPredictGroup = []; // 存储加载的图片
  }
  async show() {
    ...
    const rainPredictGroup = await preloadImages(); // 等待所有图像加载完成
    ...
    //第一张图片
    this.rainPredictEntity = viewer.entities.add({
      name: "radarAnimation",
      polygon: {
        hierarchy: new Cesium.PolygonHierarchy(positions),
        material: new Cesium.ImageMaterialProperty({
          image: rainPredictGroup[0], // 初始图片
          transparent: true,
        }),
      },
    });
    ...
    this.intervalId = setInterval(() => {
      ...
      this.rainPredictEntity.polygon.material.image = new Cesium.ConstantProperty(
        rainPredictGroup[this.currentIndex]
      );
    }, 2500);
    this.hasLoaded = true;
  }
  hide() {
    ...
    if (this.intervalId) {
      ...
      this.intervalId = null;
      viewer.entities.remove(this.rainPredictEntity);
    }
...
export default new rainPredict();
